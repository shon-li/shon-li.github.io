...
我对应上面的每个问题都单独做了一个表单（<span lang="en">sheet</span>）。又额外做了一个叫做“其他”的表单，在里面放了一个算数用的表和计算天数的表（<span lang="en">table</span>），天数是用来帮我算平均花费的。
...
这个表最多就算三个人，因为我没什么朋友吧。当每个人有自己买的东西和一起分钱的东西的时候用它算就安心一点。\\
...
如果你不熟悉使用电子自表格软件的话可能会多花点些时间。去网上搜索搜索会找到些有用的东西的。比如搜索“<span lang="en">Excel  DATEDIF</span>”这样的关键字。
